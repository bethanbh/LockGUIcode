...
{ 
  ...
  //if SOMETHING, and also the start_time isn't zero (shouldn't be I think, as gets set as current time in the PIOC handler)- initially gets set as 0 though
  {
    ...
    if (!(ADC->ADC_EMR & 0x01)) //this specific line applies when in LOW mode- but because just toggled the mode, it applies when it was in HIGH mode- ie looking for start of peak as expected :)
...
void PIOC_Handler()//Right now I am using the PIOC_handler from the internal functions to work out the interrupt. Will use a typedef function pointer. See WInterrupts.c and Stephen Prata
{
  ...
  //compare the interrupt status regigter for input/outputs to PC1 bitwise, where s
  ...
  //cavity writing remains the same
  dacc_set_channel_selection(DACC, 1);
  dacc_write_conversion_data(DACC_INTERFACE, cavity_control_signal + CAVITY_DACC_OFFSET);
  //laser writing now want to cycle through demux channels
  dacc_set_channel_selection(DACC, 0);
  dacc_write_conversion_data(DACC_INTERFACE, laser_control_signal + LASER_DACC_OFFSET);
 /* //ultimately will probably be something like this:
  selectChannel(0);
  dacc_write_conversion_data(DACC_INTERFACE, laser_control_signal + LASER_DACC_OFFSET);
  //delay(); //possibly need an amount of delay to allow the acquisition to happen- doubt this is all going faster than 3.5us but I guess this would be the solution
  selectChannel(1);
  dacc_write_conversion_data(DACC_INTERFACE, laser_control_signal_1 + LASER_DACC_OFFSET);
  //etc
  selectChannel(2);
  //end with sampling a dummy channel*/
}
