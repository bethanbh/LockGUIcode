        ...
        try:
            ...
            while time.time()< (starttime+600): 
               # print("tick")
                ...
                A0 = self.arduino.readline()
                A0 = float(A0.decode().strip())
                
                #set peak 2
                self.arduino.write(str.encode('pb!'))
                A1 = self.arduino.readline()
                A1 = float(A1.decode().strip())
                
                #set peak 3
                self.arduino.write(str.encode('pc!'))
                A2 = self.arduino.readline()
                A2 = float(A2.decode().strip())
                
                #getting the error data
                self.arduino.write(str.encode('pd!'))
                A3 = self.arduino.readline()
                A3 = float(A3.decode().strip())
                
                self.arduino.write(str.encode('pe!'))
                A4 = self.arduino.readline()
                A4 = float(A4.decode().strip())
                            
                #set peak 3
                self.arduino.write(str.encode('pf!'))
                A5 = self.arduino.readline()
                A5 = float(A5.decode().strip())
                
                #getting the error data
                self.arduino.write(str.encode('pg!'))
                A6 = self.arduino.readline()
                A6 = float(A6.decode().strip())
                
                self.arduino.write(str.encode('ph!'))
                A7 = self.arduino.readline()
                A7 = float(A7.decode().strip())
                
                #emit a signal to the main thread that has the peak position, error and time data
                #triggers the plotting/saving functions
                self.updated.emit([A0, A1, A2, A3, A4, A5, A6, A7, time.time()-starttime]) #emit a signal to say it's been updated!
